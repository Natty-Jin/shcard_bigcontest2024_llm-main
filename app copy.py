...
st.write("")
st.write("#흑돼지 #갈치조림 #옥돔구이 #고사리해장국 #전복뚝배기 #한치물회 #빙떡 #오메기떡 #말고기..🤤")
st.write("")
...
# 호출 횟수 상태 변수 초기화
if "llm_call_count" not in st.session_state.keys():
...
def clear_chat_history():
    ...
    st.session_state.llm_call_count = 0  # 호출 횟수 초기화
...
def generate_response_with_faiss(
    ...
    system_message = "Max Token은 최대 5000으로 제한해주세요 이를 어기면 안됩니다.\n"
    ...
    # 호출 횟수 제한 확인
    if st.session_state.llm_call_count < 3:
        response = model.generate_content(prompt)
        st.session_state.llm_call_count += 1  # 호출 횟수 증가
    else:
        response = "LLM 호출 횟수 제한을 초과했습니다. 추가 호출은 허용되지 않습니다."
